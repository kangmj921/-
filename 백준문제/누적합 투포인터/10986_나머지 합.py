...
# 0이 된다.
# 누적합이 M으로 나눈 나머지가 0이 아니면, M으로 나누게 만드는 경우와 M의 배수로 나누게 만드는 경우 가 있어
# 해당 나머지 값이 몇 번 먼저 나왔나가 중요하다.
# 이 때는 나머지 값이 나온 만큼의 횟수 - 1(자기 자신 제외)을 더하고
# 그 외의 경우, 즉 해당 나머지 값이 처음 나왔을 때는 해당 나머지만큼의 누적합이 나온적 있는지 확인하고 +1 한다.
#
...
for s in S_list:
    ...
    if temp:  # 누적합의 M으로 나누어 떨어지지 않음.
        ...
        if mod_dict[temp] > 1:  # 해당 나머지 값을 가진 적이 1번 보다 더 나왔다면
            # 자신을 제외한 해당 나머지 값이 나왔던 개수만큼 경우의 수 더한다.
